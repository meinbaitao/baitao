...
public class OrderBomDetailService extends
	...
	@Transactional(readOnly = false)
	public void batchInsert(List<OrderBomDetail> list) throws BusinessException {
		Map<String, OrderBomDetail> map = new HashMap<String, OrderBomDetail>();
		for(OrderBomDetail obj: list){
			String key = obj.getBoxNo()+obj.getMaterielNo();
			if(!map.containsKey(key)){
				map.put(key, obj);
			}else{
				throw new BusinessException("order.bombase.file.data.duplicate","设计单箱内物料");
			}
		}
		for(OrderBomDetail obj: list){
			orderBomDetailDao.insert(obj);
		}
	}
	...
	@Transactional(readOnly = false)
	public void deleteByOIdUp(OrderBomDetail orderBomDetail) {
		orderBomDetailDao.deleteByOIdUp(orderBomDetail);
	}
	...
	public String backCutCount(OrderBomDetail orderBomDetail) throws BusinessException{
		...
		if (materialNo.contains("YX12")||materialNo.contains("YX11")||materialNo.contains("CX07")) {
