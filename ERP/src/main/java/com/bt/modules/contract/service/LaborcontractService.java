...
public class LaborcontractService extends ConContainDetailService<LaborcontractDao,
	...
	public List<Laborcontract> findLists(Laborcontract laborcontract) {
		return laborcontractDao.findLists(laborcontract);
	}
	/**
	 * 查询我创建的
	 * @param page
	...
	public Page<Laborcontract> findUnAuditLaborcontract(Page<Laborcontract> page, Laborcontract laborcontract) {
		laborcontract.setPage(page);
		laborcontract.getExtraData().put("wfInit", CommonStatus.WORK_FLOW_INIT);
		laborcontract.getExtraData().put("wfEnd", CommonStatus.WORK_FLOW_END);
...
//		page.setList(laborcontractDao.findAuditDoneLaborcontract(laborcontract));
	...
	public String createSeriesnumber(Laborcontract entity){
		...
		String projectCode = entity.getProName();
...
//		int count = dao.getTodayCount(date);
		String number = "";
		Laborcontract l = dao.get(entity.getId());
		if(l==null){	//创建合同号
			Laborcontract laborcontract = new Laborcontract();
			laborcontract.setCreateDate(new Date());
			laborcontract.setDelFlag(Laborcontract.CON_LIST);
			int count = dao.getTodayByCondition(laborcontract);	//创建时间
			number = SeriesNumberUtils.createSeriesNumber(prefix,date, count);
		}else{	//修改合同号
			number = SeriesNumberUtils.updateContractSeriesNumber(prefix, l.getContractNo());
		}
		return number;
	}
...
//		工程名称要求:项目简称+楼栋号+承接范围+装饰工程，多栋楼用顿号表示
		String projectCode = entity.getProName();	//项目简称
		String buildings = entity.getBuildings();	//楼栋号
		String exeScope = entity.getExeScope();		//承接范围
...
//		 + "-"
	...
	public void processFinishedNotify(Map<String, Object> variables) {
		...
		//明细列表数据流程字段修改
		laborcontract.setParentid(laborcontract.getId());
		dao.updateForParentid(laborcontract);
	}
