...
//					List<HistoricIdentityLink> il = historyService.getHistoricIdentityLinksForProcessInstance(procInsId);
	...
	public String startProcess(String procDefKey, String businessTable, String businessId, String title, Map<String, Object> vars) {
		...
		// 启动流程
		vars.put(ActivitiConstants.BUSINESS_TABLE, businessTable);
		...
		//record current task
		//FIXME 其实可以把记录和追踪功能放在activiti监听器里面去
		mappingFacedService.recordCurrentTasks(procIns.getId(), vars.get(ActivitiConstants.PRIMARY_KEY).toString(), businessTable);
		auditTraceService.traceFlowStart(procIns.getId());
...
//	}
	 ...
	 * @return	封装了各种节点信息
	 */
	public InputStream tracePhoto( String executionId) {
	...
	private void setTaskGroup(Map<String, Object> vars, Set<Expression> candidateGroupIdExpressions) {
		String roles = "";
		//Fixme 避免使用 + 号拼接字符串，请使用 StringBuilder
		for (Expression expression : candidateGroupIdExpressions) {
			String expressionText = expression.getExpressionText();
			//String roleName = identityService.createGroupQuery().groupId(expressionText).singleResult().getName();
			//关闭了activiti的 identity相关的表，懒得维护两套 。
			String roleName = systemService.getRoleByEnname(expressionText).getName();
			roles += roleName;
		}
	...
	private void setCurrentTaskAssignee(Map<String, Object> vars, Task currentTask) {
		...
		if (assignee != null) {
			//使用本系统的用户表，去掉activiti的identity表 许俊雄 20160518
		/*	org.activiti.engine.identity.User assigneeUser = identityService.createUserQuery().userId(assignee).singleResult();
			...
			String userInfo  = String.format("%s-%s", assigneeUser.getName(),assigneeUser.getLoginName());
