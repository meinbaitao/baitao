...
//		System.out.println("dataScopeFilter: " + sqlString.toString());
	 ...
	 * @return 标准连接条件对象
	 */
	public String dataScopeFilter(User user,String userAlias) {
		...
		if (!user.isAdmin()){
			...
			//数据范围：仅为本人
			if (StringUtils.isNotBlank(userAlias) && !isDataScopeAll){
				for (String ua : StringUtils.split(userAlias, ",")){
					sqlString.append(" OR " + ua + ".id = '" + user.getId() + "'");
				}
				sqlString.append(extraCondition());
			}
		}
		if (StringUtils.isNotBlank(sqlString.toString())){
