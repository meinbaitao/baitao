			...
			if(falg){
				window.confirmx('确认要申请该计划吗？',function(){
					...
					if(ids){
						...
						$("#inputForm").attr("action",startUp.getRootPath()+"/a/task/repeatTeak/setTaskPlanDelay?ids="+ids);
						$("#inputForm").submit();
					}
				});
			}else{
...
<body>
		...
		<div class="list-content" >
			...
			<form:form id="searchForm" modelAttribute="taskPlan" action="${ctx}/task/repeatTeak/findByType" method="post" class="breadcrumb form-search">
				<ul class="search">
					<li>
							...
							<tbody >
								<c:forEach items="${page.list}" var="subProject" varStatus="idx" >
									...
									<tr list="taskList" index="${idx.index}">
										...
										<c:choose>
											<c:when test="${!empty subProject.actualFinishTime}">
												<td >${subProject.actualFinishTime}</td>
												<td >${subProject.finishReview}</td>
												<td >${subProject.summaryRemarks}</td>
											</c:when>
											<c:otherwise>
												<td><input type="text" readonly="readonly"  onclick="timeinput()" class="input-medium Wdate updateTaskPlan" name="taskList[${idx.index}].actualFinishTime" id="actualFinishTime${idx.index}" /></td>
												<td>${subProject.finishReview}</td>
												<td><input type="text" class="input-medium" name="taskList[${idx.index}].summaryRemarks" id="summaryRemarks" maxlength="200"/></td>
											</c:otherwise>
										</c:choose>
										<td>
											<c:choose>
