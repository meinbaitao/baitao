$(document).ready(function() {
				...
				if(falg){
					window.confirmx('确认要删除该计划吗？',function(){
						...
						if(ids){
							...
							var url=startUp.getRootPath()+"/a/task/taskPlan/deletetaskQuotaPlan";
			...
			$("#submitTaskQuotaPlan").live("click",function(){
				...
				if(falg){
					window.confirmx('确认要提交该计划吗？',function(){
						...
						if(ids){
							ids=ids.substring(0,ids.lastIndexOf(","));						
							var url=startUp.getRootPath()+"/a/task/taskPlan/setTaskQuotaStatus";
							window.location.href=url+"?id="+ids+"&deptId="+deptId;
						}
					});
				}else{
	...
	$(function(){
		...
		$(".saveTaskQuotaPlan").bind("click",function(){
			...
			$("#inputForm").attr("action",startUp.getRootPath()+"/a/task/taskPlan/saveTaskQuotaPlan");
		});
		/**
		 * 修改指标型计划
		 */
		$(".updateTaskQuotaPlan").bind("click",function(){
			$("#inputForm").attr("action",startUp.getRootPath()+"/a/task/taskPlan/updateTaskQuotaPlan");
		});
	...
	$("#subProjectList tr td>select").live("select2-opening",function(){
		...
		if(type){
			...
			if(status==1){
				...
				url = "/a/task/taskPlan/findResponsiblePerson?deptId="+deptId;
			}else if(status==2){
				url = "/a/task/taskPlan/findReviewOffice";
			}
