  ...
  <div class="list-content" >
      <div class="nav-operate" >
		<ul class="operationg-new" >
			<li>
				<form id="returnForm" action="${ctx}/accident/accidentManage/list?type=1" method="post" >
     ...
     <div class="form-content">
	  <form:form id="inputForm" modelAttribute="accidentManage" action="${ctx}/accident/accidentManage/${flag eq 'add'?'save':'update'}" method="post" class="form-horizontal magbtm0"> 
		<ul>
       	  <li class="cols4">
             <span class="tab-name">单据编号：</span>
             <label>
             	 <input type="hidden" name="id" id="accidentId" value="${accidentManage.id}"/>
             	 <input type="hidden" name="type" id="accidentType" value="1"/>
                 ...
                 <form:select id="logisticsId" path="orderId" class="input118" tabindex="-1">
             	 	<option value="">请选择单据编号</option>
				 	<c:forEach items="${logisticsSendList}" var="obj" varStatus="idxStatus">
						 <option showSuppliersName="${obj.showSuppliersName }" sendFactory="${obj.sendFactory }" seriesnumber="${obj.seriesnumber }" requireDate="${obj.requireDate}" factSendDate="${obj.factSendDate}" value="${obj.id}" <c:if test="${obj.id==accidentManage.orderId}">selected="selected"</c:if>>${obj.seriesnumber}</option>
				 	</c:forEach>
				 </form:select>
             </label>
          </li>
          <li class="cols4">
             ...
             <label>
                  <input type="text"  id="sendCC" value="${accidentManage.sendCC}" disabled="disabled" class="input70"/>
             </label>
          </li>
          <li class="cols4">
             <span class="tab-name">要求到货日期：</span>
             <label>
                 <input type="text"  id="requireDate" value="${accidentManage.requireDate}" disabled="requireDate" class="input115"/>
             </label>
          </li>
          <li class="cols4">
             <span class="tab-name">实际到货时间：</span>
             <label>
                 <input type="text"  id="factArriveDate" name="factArriveDate" value="${accidentManage.factArriveDate}" readonly="readonly"  onclick="timeinput()"  class="input115 required Wdate"/>
             </label>
          </li>
	  ...
	  <table id="contentTable" class="table table-striped table-bordered table-condensed  table-eidt">
			<thead>
			   <tr list="amdList" index="0">
					...
					<th type="hidden" id="orderBomBaseId"  name="orderBomBaseId" ></th>
			   </tr>	
			</thead>
			...
			<c:if test="${flag eq 'edit'}">
				...
				<c:forEach items="${accidentManage.amdList}" var="item" varStatus="idx" >
					<tr list="amdList" index="${idx.index}">
						...
						<td type="hidden" id="amdList[${idx.index}].orderBomBaseId"  name="amdList[${idx.index}].orderBomBaseId" value="${item.orderBomBaseId}" ></td>
					 </tr>
		 		</c:forEach>
				</tbody>
			</c:if>
