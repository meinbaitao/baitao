...
public class ProjectService extends CrudService<ProjectDao, Project>  implements ProcessDataHandler{
	...
	public Page<Project> findPage(Page<Project> page, Project project) {
		...
		project.getSqlMap().put("dsf", dataScopeFilter(project.getCurrentUser(),"uos,uc,udl,um,uo,ul,ums,ucs,up,ur"));
...
//	}
	...
	@Transactional(readOnly = false)
	public void insert(Project project) throws BusinessException {
		project.setIsNewRecord(true);
		this.validateProjectNameDuplicate(project);
		project.preInsert();
		super.dao.insert(project);
	}
	 ...
	 */
	public void validateProjectNameDuplicate(Project project) throws BusinessException{
		Project vproject = this.getProjectByName(project.getName());
		if(vproject!=null&&!vproject.getId().equals(project.getId())){
			throw new BusinessException("project.name.duplicate",project.getName());
		}
	}
	 ...
	 * @return
	 */
	public List<Project> gbProjectByCode(Project project){
		project.setGbCondition("code");
		return this.gbProjectByCondition(project);
	}
	/**
	 * 根据条件对项目数据进行分组
	 * @param project
	...
	public List<Project> gbProjectByCondition(Project project){ 
		project.setWorkflowstatus(CommonStatus.WORK_FLOW_END);
		return projectDao.gbProjectByCondition(project);
	}
	 ...
	 * @return
	 */
	public Page<Project> findPageByStatusOnData(Page<Project> page, Project entity) {
		entity.setPage(page);
		entity.getSqlMap().put("dsf", dataScopeFilter(UserUtils.getUser(), "uos,uc,up,ur"));
		page.setList(projectDao.findListByStatus(entity));
		return page;
	}
	...
	protected String extraCondition() {
		return " OR " + "a.design_leader_bid LIKE" + "'%" + UserUtils.getUser().getId() + "%'";
	}
	
	public Page<Project> findPageByDesignLeaderBid(Page<Project> page, Project entity) {
		entity.setPage(page);
		entity.getSqlMap().put("dsf", dataScopeFilter(UserUtils.getUser(), "uos,uc"));
