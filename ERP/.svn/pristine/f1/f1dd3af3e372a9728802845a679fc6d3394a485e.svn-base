$(document).ready(function() {
				...
				if(falg){
					window.confirmx('确认要删除该计划吗？',function(){
						...
						if(ids){
							...
							var url=startUp.getRootPath()+"/a/task/taskPlan/deletetaskPlan";
			...
			$("#submitTaskPlan").live("click",function(){
				...
				if(falg){
					window.confirmx('确认要提交计划吗？',function(){
						...
						if(ids){
							ids=ids.substring(0,ids.lastIndexOf(","));						
							var url=startUp.getRootPath()+"/a/task/taskPlan/setTaskStatus";
							window.location.href=url+"?id="+ids+"&deptId="+deptId;
						}
					});
				}else{
	...
	$(function(){
		...
		$(".saveTaskPlan").bind("click",function(){
			$("#inputForm").attr("action",startUp.getRootPath()+"/a/task/taskPlan/saveTaskPlan");
		});
		/**
		 * 修改任务型计划
		 */
		$(".updateTaskPlan").bind("click",function(){
			$("#inputForm").attr("action",startUp.getRootPath()+"/a/task/taskPlan/updateTaskPlan");
		});
	});
