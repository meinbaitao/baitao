		...
		<li>
			...
			<label>
				...
				<option value="-1">===请选择===</option>
					<c:forEach items="${suppliersList }" var="obj">
						<option value="${obj.suppliersName}"
						<c:if test="${payment.paymentObject ==obj.suppliersName }">selected="selected"</c:if>>
						${obj.suppliersName}</option>
					</c:forEach>
				</select>
			</label>
		</li>
...
<%-- 	   <div class="table_scroll">
	   <table id="contentTable1" class="table table-striped table-bordered table-condensed table-eidt">
			...
			<tbody id="paymentList" copy="false">
				...
				<c:forEach items="${payment.manufList}" var="detail" varStatus="idx">
					<tr list="manufList" index="${idx.index }">
						<td type="hidden" name="manufList[${idx.index}].id" value="${detail.id}" class="hide"></td>
						<td type="hidden" name="manufList[${idx.index}].mappingId" value="${detail.mappingId}" class="hide"></td>
						<td class="check">
							<input type="checkbox" id="${detail.mappingId}" name="manufList[${idx.index}].mappingId" value="${detail.mappingId}">
						</td>
						<td type="select" class="required input-medium" readonly="true" name="manufList[${idx.index}].logisticSend" >${detail.logisticSend}</td>
						<td type="text" class="input-medium" readonly="true" name="manufList[${idx.index}].transportationNum" >${detail.transportationNum}</td>
						<td type="text" class="input-medium" readonly="true" name="manufList[${idx.index}].startArea" >${detail.startArea}</td>
						<td type="text" class="input-medium" readonly="true" name="manufList[${idx.index}].realWeight" >${detail.realWeight}</td>
						<td type="text" class="input-medium" readonly="true" name="manufList[${idx.index}].settleWeight" >${detail.settleWeight}</td>
						<td type="text" class="input-medium" readonly="true" name="manufList[${idx.index}].contractPrice" >${detail.contractPrice}</td>
						<td type="text" class="input-medium" readonly="true" name="manufList[${idx.index}].loadPrice" >${detail.loadPrice}</td>
						<td type="text" class="input-medium" readonly="true" name="manufList[${idx.index}].unloadTotalPrice" >${detail.unloadTotalPrice}</td>
						<td type="text" class="input-medium" readonly="true" name="manufList[${idx.index}].sumPrice" >${detail.sumPrice}</td>
					 </tr>
		 		</c:forEach>			
			</tbody>
		</table>
