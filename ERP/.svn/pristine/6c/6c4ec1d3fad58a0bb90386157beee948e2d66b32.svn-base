...
<body>
	<!-- 流程提交 -->
	<form:form style="display:none;" id="submitACTForm" action="${ctx}/purchase/purchase/submitACTFL" onsubmit="javascript:return validateSubmit();" method="post" class="breadcrumb form-search">
		<input id="ids" name="sdIDs" type="hidden" value=""/>
	</form:form>
	<!-- 流程通过，进行下一步 -->
	<form:form style="display:none;" id="passACTForm" action="${ctx}/purchase/purchase/passACTFL" onsubmit="javascript:return validateSubmit();" method="post" class="breadcrumb form-search">
		<input name="wfID" value="${wfID }" type="text">
	</form:form>
	<!-- 驳回流程 -->
	<form:form style="display:none;" id="rebutACTForm" action="${ctx}/purchase/purchase/rebutACTFL" method="post" class="breadcrumb form-search">
	...
	<div class="form-actions">
		<!-- 返回 -->
		<a href="${ctx}/purchase/purchase/purchaseFLList" class="btn btn-primary">返回</a>
		<!-- 保存 -->
	...
	<div class="list-content">
			...
			<tr>
				...
				<c:choose>
					<c:when test="${p.status eq isStatus}">
						...
						<td><input style="width:80px;height:11px;" id="price${p.id}" type="text" value=""/></td>
					</c:when>
					<c:otherwise>
						<!-- <td>&nbsp;</td> -->
		 ...
		 <div class="div_right" id="CPDiv">
			 <form id="saveForm" modelAttribute="" action="${ctx}/purchase/purchase/savePurchaseFL" method="post" onsubmit="javascript:return validateSubmitDemand();" class="form-horizontal">
		        ...
		        <ul>
			                  	...
			                  	<%-- <c:forEach items="${fns:getSuppByType('supp_gys')}" var="obj">
									<c:choose>
										<c:when test="${fns:escapeHtml(obj.id) eq supp.id}">
											<option selected="selected" contactphone="${fns:escapeHtml(obj.contactphone)}" value="${fns:escapeHtml(obj.id)}" >${fns:escapeHtml(obj.name)}</option>
										</c:when>
										<c:otherwise>
		          ...
		          <li>
		             <span class="tab-name">
		             	联系人：</span>
		             	<label>
		                  <%-- <input id="contact" disabled="disabled" type="text" value="${supp.businessPreson }"/> --%>
		             </label>
		          ...
		          <li>
		             ...
		             <label>													<!-- address -->
		                 <%-- <input id="cpname" disabled="disabled" type="text" title="${supp2.address }" value="${supp2.address }"/> --%>
		             </label>
		             <%-- <label>
