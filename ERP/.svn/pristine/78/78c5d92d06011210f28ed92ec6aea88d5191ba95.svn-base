...
<head>
	...
	<script type="text/javascript" src="${ctxStatic}/modules/comm/jquery.jqprint-0.3.js?tsf=${jsVersion}"></script>
	...
	/* $(function(){
		...
		$("#outsideWallOrientation").unbind("change").bind("change",function(){
			...
			inputForm.attr('action',startUp.getRootPath()+"/a/orbom/orderBomDetail/list?orderType=11&orderBomId="+orderBomId+"&outsideWallOrientation="+outsideWallOrientation);
			inputForm.submit();				
		});
	}); */
	</script>
				...
				<tbody>
					...
					<c:forEach items="${listTwo}" var="items" varStatus="idx">
		              <tr>
		               ...
		               <td type="text" class="input-medium" name="list[${idx.index}].standardL1" id="standardL1">${items.standardL1 }</td>
		               <td type="text" class="input-medium" name="list[${idx.index}].standardL2" id="standardL2">${items.standardL2 }</td>
