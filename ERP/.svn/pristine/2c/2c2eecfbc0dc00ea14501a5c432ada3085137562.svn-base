...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			var zNodes=[
		            <c:forEach items="${categoryList}" var="tpl">{id:'${tpl.id}', pId:'${not empty tpl.parent?tpl.parent.id:0}', name:"${tpl.name}", url:"${ctx}/task/taskPlan/findByOffice/${not empty tpl.id?tpl.id:tpl.parent.id}/${tpl.grade}", target:"officeMainFrame"},
		            </c:forEach>];
		...
		function wSize(){
			...
			$(".ztree").css({"overflow":"hidden","overflow-x":"hidden","overflow-y":"hidden"});
