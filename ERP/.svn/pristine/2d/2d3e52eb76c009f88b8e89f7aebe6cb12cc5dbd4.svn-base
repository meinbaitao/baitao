 ...
 */$(function(){
	...
	$("#addContract").on("click",function(){
		 window.location.href=startUp.getRootPath()+"/a/finance/transportationPayment/form";
	});
	...
	$("#paymentList tr td>select[name*='logisticSend']").live("select2-opening",function(){
		var url = "/a/finance/transportationPayment/findFinishSettlement";
		var obj =this;
	...
	$("#paymentList tr td>select[name*='logisticSend']").live("change", function(){
		...
		var url = "/a/finance/transportationPayment/getSettlementByNum?logisticSend="+seriesNumber;
		startUp.getAsyncData(url, function(data){
			if(data){
	...
	$("#paymentObject").live("change", function(){
		var supplierName = $("#paymentObject").val();
		var url = "/a/finance/transportationPayment/getSupplierByName?supplierName="+supplierName;
