	...
	<script type="text/javascript">
		...
		function deleteData(id){
			window.confirmx('确认要删除？',function(){
				url = "/a/qualityitem/delete?id="+id;
				startUp.getAsyncData(url,function(data){
					 window.location.href=startUp.getRootPath()+"/a/qualityitem/list";
				});
			});
		}
	</script>
		...
		<tbody>
			<c:forEach items="${page.list}" var="qualityitem" varStatus="1">
				<tr pid="0" id="${qualityitem.id}" class="0 first-level" haschild="true" previd="" islastone="true" depth="1">
					...
					<a href="javascript:void(0)" name="${qualityitem.id}" onclick="deleteData('${qualityitem.id}')">删除</a>
					</td>
				 </tr>
			 </c:forEach>
			</tbody>
